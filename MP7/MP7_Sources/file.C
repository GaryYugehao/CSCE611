...
File::File(inode * _inode) {
	...
	blk_idx = 0;
	//Some initialization work to do.
}
...
int File::Read(unsigned int _n, char * _buf) {
	...
	//When it isn't the end of the file.
	while(not EoF() && read_cnt < _n){
		//Read information of the file in current block.
		FILE_SYSTEM->disk->read(cur_inode->loc[blk_idx++], (unsigned char*)temp_buffer);
		//If the file opccupied all space
		if((_n-read_cnt) > 512){
			memcpy(_buf+read_cnt, temp_buffer, 512);
			read_cnt += 512;
		//If only occupy only a part of the space.
		}else{
...
void File::Rewrite() {
    ...
    //assert(false);
	//release the occupied blocks space of the file.
	blk_idx = 0;
...
bool File::EoF() {
    ...
    //assert(false);
	//IF the length of current inode is 0, or we exceed the block index, return True.
	if(cur_inode->len ==0 ||((cur_inode->len)<blk_idx)){
		return true;
	}
	//Otherwise, not the end of the file.
	return false;
}
